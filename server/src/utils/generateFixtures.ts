...
async function createSampleDocuments() {
  ...
  // Page 1: Role Overview
  const page1 = doc1.addPage([600, 800]);
  page1.drawText('UNTHINKABLE - SENIOR SOFTWARE ENGINEER POSITION', {
    x: 50,
    y: 740,
    size: 16,
    font: fontBold,
  ...
  const page2Body = [
    ...
    'Total annual compensation for this role is Rs 3,00,000 with additional performance incentives.',
  ...
  for (const line of page2Body) {
    page2.drawText(line, { x: 50, y, size: 10, font: fontRegular, color: rgb(0.1, 0.1, 0.1) });
    y -= 22;
  }

  const pdfBytes1 = await doc1.save();
  await fs.writeFile(path.join(outDir, 'sample-job-description.pdf'), pdfBytes1);
  console.log('Successfully created test fixture: test-fixtures/sample-job-description.pdf');
}
